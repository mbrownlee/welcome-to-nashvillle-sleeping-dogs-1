...
document
	.querySelector("#search-btn__concerts")
	.addEventListener("click", () => {
		let apiUrl = app_keys.app_url;
		let apiKey = app_keys.app_key;

		const concertUserSearch = document.querySelector("#search-input__concerts")
			.value;
		document.querySelector(".search-results__container").innerHTML = "";
		fetch(
			`${apiUrl}${apiKey}&keyword=${concertUserSearch}&local=*&city=Nashville&stateCode=TN`
		)
...
function makeConcertListComponent(concert) {
	...
	return `
			<section class="concert-result">
				<div class="concert-name" id="concert__${concert.id}">${concert.name}</div>
